...
class MQ
  class << self
    def run
      ...
      sub.subscribe_to 'octolux/cmd/ac_charge_priority', &method(:ac_charge_priority_cb)
    ...
    def ac_charge_priority_cb(data, *)
      LOGGER.info "MQ cmd/ac_charge_priority => #{data}"
      r = lux_controller.charge_priority(bool(data))
      lux_controller.close
      sub.publish_to('octolux/result/ac_charge_priority', r ? 'OK' : 'FAIL')
    rescue LuxController::SocketError
      sub.publish_to('octolux/result/ac_charge_priority', 'FAIL')
    end
